...
// global Escape listener only while open
watch(open, (isOpen) => {
  if (isOpen) {
    query.value = ''
    active.value = 0
...
<template>
  <div class="absolute bottom-4 left-4 z-40 flex max-w-[calc(100%-2rem)] flex-col-reverse items-start gap-2">
    <!-- the + button is the anchor the panel grows out of -->
    <button
      type="button"
      title="Insert elements (⌘E)"
