...
- **Users / auth** (`useAuth`, `useUsers`, `server/auth.mjs`): roles `admin | editor | contributor`; `canBuild` = admin|editor. scrypt-hashed passwords, HttpOnly session cookie, single-use hashed invite tokens (accepted at `/admin/invite/:token`). The server hard-gates publish (contributor forbidden) and user management (admin only); **the build/structure restriction for contributors is UI-enforced only** — see `SECURITY.md` (finding S1) for that intentional gap.
